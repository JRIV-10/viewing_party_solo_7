...
RSpec.describe MovieSearchFacade do
  ...
  describe "#movies" do 
    it "can search for top voted movies and call the correct api end point" do 
      ...
      stub_request(:get, "https://api.themoviedb.org/3/discover/movie?api_key=4c5a041db10b9e21e2c9287447a724f4&include_adult=false&include_video=false&language=en-US&page=1&sort_by=vote_average.desc").
        with(
          query: {
    ...
    it "can search movies based off keywords" do 
      ...
      stub_request(:get, "https://api.themoviedb.org/3/search/movie?api_key=4c5a041db10b9e21e2c9287447a724f4&include_adult=false&language=en-US&page=1&query=Kung%20Fu%20Panda").
        with(
          query: {
            "api_key" => Rails.application.credentials.tmdb[:key]
          }
